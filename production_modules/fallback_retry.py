# Try the LLM classification → retry when certain failures happen → simplify the prompt on retry → if everything still fails, return a safe fallback classification instead of crashing.
...
# @retry = "Retry this function up to 3 times when RateLimitError or ValidationError occurs, with exponential waiting and retry logging."
@retry(
